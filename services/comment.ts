...
export class CommentService {
  ...
  static async deleteComment(commentId: string, userId: string): Promise<void> {
    const result = await transaction(async (client) => {
      ...
      const deleteResult = await client.query("UPDATE comments SET is_deleted = true WHERE id = $1 AND user_id = $2", [
        commentId,
        userId,
      ])

      if (deleteResult.rowCount === 0) {
        throw errors.notFound("Comment not found or you don't have permission to delete it")
      }

      // Get post ID for cache invalidation
      const postResult = await client.query("SELECT post_id FROM comments WHERE id = $1", [commentId])
      return postResult.rows[0]?.post_id
    })
