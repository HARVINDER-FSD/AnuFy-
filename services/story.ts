...
export class StoryService {
  ...
  static async deleteStory(storyId: string, userId: string): Promise<void> {
    const result = await query("SELECT media_url FROM stories WHERE id = $1 AND user_id = $2", [storyId, userId])

    if (result.rows.length === 0) {
      throw errors.notFound("Story not found or you don't have permission to delete it")
    }
    ...
    await transaction(async (client) => {
      // Archive the story
      await client.query("UPDATE stories SET is_archived = true WHERE id = $1", [storyId])
