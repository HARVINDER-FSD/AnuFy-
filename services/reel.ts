...
export class ReelService {
  ...
  static async deleteReel(reelId: string, userId: string): Promise<void> {
    const result = await query("SELECT video_url, thumbnail_url FROM reels WHERE id = $1 AND user_id = $2", [
      reelId,
      userId,
    ])

    if (result.rows.length === 0) {
      throw errors.notFound("Reel not found or you don't have permission to delete it")
    }
    ...
    await transaction(async (client) => {
      // Delete the reel
      await client.query("DELETE FROM reels WHERE id = $1", [reelId])
