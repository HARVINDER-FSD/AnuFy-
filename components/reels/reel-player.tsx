...
export function ReelPlayer({ reel, isActive, onLike, onComment, onShare, onBookmark, onDelete, currentUserId }: ReelPlayerProps) {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-black">
      ...
      {/* Top right controls - only 3-dot menu, fully overlaid */}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="ghost" 
              size="icon" 
              className="text-white hover:bg-transparent w-10 h-10 p-0"
            >
              <MoreHorizontal className="h-6 w-6 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-48 z-50 bg-white dark:bg-gray-800">
            {isOwner ? (
              <DropdownMenuItem
